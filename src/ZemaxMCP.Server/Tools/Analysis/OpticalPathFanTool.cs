...
public class OpticalPathFanTool
{
        ...
        try
        {
            return await _session.ExecuteAsync("OpdFan", null, system =>
            {
                ...
                try
                {
                    ...
                    var results = analysis.GetResults() ?? throw new InvalidOperationException("OPD Fan returned no results object.");
                    ...
                    try
                    {
                        ...
                        if (!File.Exists(tempFile) || new FileInfo(tempFile).Length == 0)
                            throw new InvalidOperationException("OPD Fan produced no text output.");
    ...
    private static RayFanData ParseOpdFanTextFile(string filePath)
    {
        ...
        foreach (var line in lines)
        {
            ...
            {
                SaveCurrentFan(ref currentTangential, ref currentSagittal, currentFanType, wavelengths, pupils, opdColumns);
                ...
                if (!isTangential && !isSagittal)
                    throw new FormatException($"Unrecognized OPD Fan section header: '{trimmed}'.");
                ...
                else
                {
                    if (currentFieldNumber == 0 || newFieldNumber != currentFieldNumber)
                        throw new FormatException("Sagittal OPD Fan section does not match the active field.");
            ...
            if (trimmed.StartsWith("Pupil", StringComparison.OrdinalIgnoreCase))
            {
                ...
                for (int j = 1; j < parts.Length; j++)
                {
                    if (!double.TryParse(parts[j], NumberStyles.Float, CultureInfo.InvariantCulture, out double w))
                        throw new FormatException($"Could not parse OPD Fan wavelength '{parts[j]}'.");
                    waveList.Add(w);
                }
                if (waveList.Count == 0)
                    throw new FormatException("OPD Fan data header contained no wavelengths.");
                wavelengths = waveList.ToArray();
                opdColumns = wavelengths.Select(_ => new List<double>()).ToList();
                inData = true;
                continue;
            }
            ...
            if (values.Length < 1 + wavelengths.Length)
                throw new FormatException("OPD Fan data row has fewer values than the wavelength header.");

            pupils.Add(pupil);
            for (int j = 0; j < wavelengths.Length; j++)
            {
                if (!double.TryParse(values[j + 1], NumberStyles.Float, CultureInfo.InvariantCulture, out double opd))
                    throw new FormatException($"Could not parse OPD Fan value '{values[j + 1]}'.");
        ...
        if (currentFieldNumber > 0)
            AddCompletedField(fields, currentFieldNumber, currentFieldValue, currentTangential, currentSagittal);
        if (fields.Count == 0)
            throw new FormatException("OPD Fan text output contained no parseable field data.");
        ...
        RayFanCurveData? tangential, RayFanCurveData? sagittal)
    {
        if (tangential == null || sagittal == null)
            throw new FormatException($"OPD Fan field {fieldNumber} did not contain both tangential and sagittal data.");
    ...
    private static void SaveCurrentFan(ref RayFanCurveData? tangential, ref RayFanCurveData? sagittal,
        string fanType, double[]? wavelengths, List<double> pupils, List<List<double>> opdColumns)
    {
        if (wavelengths == null || pupils.Count == 0) return;
        if (opdColumns.Count != wavelengths.Length || opdColumns.Any(c => c.Count != pupils.Count))
            throw new FormatException("OPD Fan data columns are inconsistent with the pupil/wavelength dimensions.");
        ...
        else throw new FormatException("OPD Fan data appeared before a tangential/sagittal section header.");
    }

    private static int ParseRequiredFieldNumber(string line)
    {
        var idx = line.IndexOf("number", StringComparison.OrdinalIgnoreCase);
        if (idx < 0) throw new FormatException($"OPD Fan field number is missing: '{line}'.");
        ...
        if (parts.Length == 0 || !int.TryParse(parts[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out int num) || num <= 0)
            throw new FormatException($"Could not parse OPD Fan field number from '{line}'.");
        return num;
    }
    ...
    private static double ParseRequiredFieldValue(string line)
    {
        ...
        if (idx < 0) throw new FormatException($"OPD Fan field value is missing: '{line}'.");
        ...
        if (parts.Length == 0 || !double.TryParse(parts[0], NumberStyles.Float, CultureInfo.InvariantCulture, out double val))
            throw new FormatException($"Could not parse OPD Fan field value from '{line}'.");
        return val;
    }
}
