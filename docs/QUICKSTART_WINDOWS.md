...
For a second PC, tick **Share with a trusted LAN computer**, then choose **Copy secure setup**. On the AI-client computer, paste the copied JSON directly into **Remote MCP address**; the launcher separates the endpoint and access token for you. If Windows Firewall asks, allow the selected port only for the trusted private network. The Host refuses LAN mode without a token and rejects unrelated browser origins, but it is still intended for a trusted private network—not direct internet exposure. One live OpticStudio service deliberately has one control-lease owner at a time; disconnect the first client or allow its lease to expire before moving control to another client.
...
Google Antigravity uses `~/.gemini/config/mcp_config.json`; its remote MCP schema requires `serverUrl`, not `url`. The launcher writes that exact schema, including the Bearer authorization header, and preserves unrelated servers. Kimi Code uses `~/.kimi-code/mcp.json` (or `$KIMI_CODE_HOME/mcp.json`) and WorkBuddy uses `~/.workbuddy/mcp.json`. For another HTTP-capable agent, choose **Copy generic HTTP MCP JSON** and paste the result into that agent's documented MCP settings.

Claude Desktop is configured through the packaged `ZemaxMCP.ClientProxy.exe`. Claude starts this small local stdio process, which forwards requests to the HTTP endpoint shown in the launcher; this also works when OpticStudio is on the second computer. Each proxy process supplies its own client-instance identifier so concurrent local client processes do not collapse into one OpticStudio lease identity. Codex honors `$CODEX_HOME`, Kimi honors `$KIMI_CODE_HOME`, and the launcher displays the exact configuration file inspected for every supported client. A client is marked **Configured** only when that file points to the current endpoint.
...
Use **Updates** in the launcher to download and apply the current GitHub release. The launcher accepts an update only after verifying its RSA-signed manifest, expected release version and asset name, file size, and SHA-256. A separate updater keeps a temporary backup, removes superseded program files that are absent from the new package, retains `logs` and `snapshots`, and restores the previous installation if replacement fails. Each release includes the self-contained .NET 10 Host, the net48 ZOS-API Worker, launcher, client proxy, installer, and updater. Your OpticStudio installation and AI-client configuration are retained outside the application folder. The launcher retries the Host after an unexpected exit; the Host independently faults and recreates an unhealthy Worker generation when private-RPC recovery requires it.
...
Use **Open logs** in the launcher for both Host and Worker diagnostics; no command prompt is needed to locate or inspect logs.
