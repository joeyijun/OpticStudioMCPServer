...
$domainLookup = [regex]::Match($toolset, 'public static string GetDomainId\(string toolName\)(?<body>[\s\S]*?)\n    }\n\n    public static Domain').Value
...
$profileDomains = @{
    'basic-viewing' = @('system', 'analysis', 'administration')
...
$profiles = @{
    'basic-viewing' = @{ present = @('zemax_status', 'zemax_spot_diagram', 'zemax_tool_catalog'); absent = @('zemax_set_surface', 'zemax_get_nsc_objects', 'zemax_optimize', 'zemax_open_file') }
...
foreach ($profile in $profiles.Keys) {
    ...
    foreach ($tool in $profiles[$profile].present) {
        if ($domainByTool[$tool] -notin $profileDomains[$profile]) { throw "Toolset '$profile' must include '$tool'." }
    }
    foreach ($tool in $profiles[$profile].absent) {
        if ($domainByTool[$tool] -in $profileDomains[$profile]) { throw "Toolset '$profile' must exclude '$tool'." }
    }
}
